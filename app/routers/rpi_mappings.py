...
async def get_stats(project: ValidProject, db: DBSession):
...
async def list_rpi_mappings(
    ...
    db: DBSession,
    status: str | None = Query(None, description="approved | in_review | draft"),
...
async def get_rpi_mapping(rpi_id: int, project: ValidProject, db: DBSession):
...
async def create_rpi_mapping(
    payload: RPIMappingCreate, project: ValidProject, db: DBSession
):
...
async def delete_rpi_mapping(rpi_id: int, project: ValidProject, db: DBSession):
